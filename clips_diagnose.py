...
def load_disease_info(lang):
    ...
    try:
        with open(desc_path, "r", encoding='utf-8-sig') as f:
            ...
            for row in reader:
                disease_key = row[disease_col].lower().strip().replace(" ", "_")
...
def load_symptom_disease_mapping(lang):
    ...
    try:
        with open(mapping_path, "r", encoding='utf-8-sig') as f:
            ...
            for row in reader:
                disease = row[disease_col].lower().strip().replace(" ", "_")
                symptoms = [s.strip().replace(" ", "_").lower() for s in [row.get(f'الأعراض_{i}', '') or row.get(f'Symptom_{i}', '') for i in range(1, 18)] if s.strip()]
                key_symptoms = symptoms[:2] if len(symptoms) >= 2 else symptoms
    ...
    try:
        with open(mapping_path, "r", encoding='utf-8-sig') as f:
            reader = csv.DictReader(f)
            disease_col = next((col for col in reader.fieldnames if col.lower() == 'disease'), None)
            if not disease_col:
                raise KeyError(f"Colonnes 'disease' manquantes dans {mapping_path}")
            print(f"CSV headers: {reader.fieldnames}", file=sys.stderr)
            for row in reader:
                disease = row[disease_col].lower().strip().replace(" ", "_")
                symptoms = [s.strip().replace(" ", "_").lower() for s in [row.get(f'Symptom_{i}', '') for i in range(1, 18)] if s.strip()]
                # Assume first two non-empty symptoms are key, rest are optional (mimics CLIPS rules)
                key_symptoms = symptoms[:2] if len(symptoms) >= 2 else symptoms
                optional_symptoms = symptoms[2:] if len(symptoms) > 2 else []
                symptom_disease_map[disease] = {'key': key_symptoms, 'optional': optional_symptoms}
                print(f"Loaded symptoms for {disease}: key={key_symptoms}, optional={optional_symptoms}", file=sys.stderr)
...
def diagnose(symptoms, lang):
    ...
    for disease, symptom_rules in symptom_disease_map.items():
        ...
        # Check if all key symptoms are present
        if key_symptoms.issubset(input_symptoms):
            # Bonus for matching optional symptoms
...
if __name__ == "__main__":
    ...
    try:
        ...
        symptoms = [s.strip().replace(" ", "_").lower() for s in data.get("symptoms", [])]
