...
//     try {
//       // Utiliser le nouvel endpoint pour les patients
...
//     const slots = [];
//     const now = new Date(); // Current date and time (e.g., April 24, 2025, 11:36 AM)
//     const endDate = addDays(now, 30); // Créneaux pour les 30 prochains jours
...
//       // Adjust start time ONLY for the current day
...
//         // For the current day, only include slots that are not in the past
//         if (date.toDateString() === now.toDateString()) {
...
//           }
//         } else {
//           // For future days, include all slots within the doctor's hours
//           slots.push({
//             title: `Disponible à ${format(currentTime, 'HH:mm')}`,
//             start: new Date(currentTime),
//             end: addMinutes(new Date(currentTime), 30),
...
//                   step={30}
//                   timeslots={2}
//                   min={new Date(0, 0, 0, 8, 0)}  // Début à 8h
//                   max={new Date(0, 0, 0, 20, 0)} // Fin à 20h
//                 />
//               )}
//             </div>
