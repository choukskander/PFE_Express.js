...
//   const [forums, setForums] = useState([]); // Ensure initial state is an array
...
//   // Fetch users, appointments, and forums from the API
//   useEffect(() => {
...
//           headers: { Authorization: `Bearer ${token}` },
//         });
//         console.log('API Response (forums):', forumsResponse.data);
//         // Ensure forums is an array, handle nested data if necessary
...
//         setUsers([]);
//         setAppointments([]);
//         setForums([]); // Set to empty array on error
//         Swal.fire({
//           icon: 'error',
//           title: 'Erreur',
...
//   // Debug forums state after updates
//   useEffect(() => {
//     console.log('Current forums state:', forums);
//   }, [forums]);
...
//         { headers: { Authorization: `Bearer ${token}` } }
//       );
//       console.log('Doctor validation response:', response.data);
//       setUsers(
//         users.map((user) =>
//           user._id === userId ? { ...user, validated: true } : user
//         )
//       );
//       showNotification('Licence du médecin validée avec succès. Un email a été envoyé au médecin.');
//     } catch (err) {
//       console.error('Error validating doctor:', err.response?.data?.message || err.message);
//       Swal.fire({
//         icon: 'error',
//         title: 'Erreur',
...
//         showNotification('Forum supprimé avec succès');
//       } catch (err) {
//         Swal.fire({
//           icon: 'error',
//           title: 'Erreur',
//           text: err.response?.data?.message || 'Impossible de supprimer le forum.',
//           toast: true,
//           position: 'top-end',
//           timer: 3000,
//           timerProgressBar: true,
...
//               {activeSection === 'forums' && 'Gestion des Forums'}
//             </h1>
...
//                   <Button variant="primary" size="sm" onClick={() => navigate('/create-forum')}>
//                     <i className="fas fa-plus me-2"></i> Créer un forum
//                   </Button>
//                 </div>
//                 <div className="table-responsive">
//                   <Table striped bordered hover>
...
//                         forums.map((forum) => (
//                           <tr key={forum._id}>
//                             <td>{forum.title || 'N/A'}</td>
//                             <td>{forum.description || 'N/A'}</td>
//                             <td>{forum.createdBy?.name || 'Inconnu'}</td>
//                             <td>{new Date(forum.createdAt).toLocaleDateString('fr-FR')}</td>
//                             <td>
//                               <Button
//                                 variant="link"
//                                 className="text-danger p-1"
...
//                 <div className="d-flex justify-content-between align-items-center border-top pt-3">
//                   <span className="text-muted">
//                     Affichage de 1 à {forums.length} sur {forums.length} résultats
//                   </span>
//                   <div className="d-flex gap-2">
//                     <Button variant="outline-secondary" size="sm">
//                       Précédent
...
const AdminDashboard = () => {
  ...
  const handleViewForumResponses = (forumId) => {
    navigate(`/forum-responses/${forumId}`);
  };
                          ...
                          <tr key={forum._id}>
                            ...
                            <td>
                              <Button
                                ...
                                onClick={() => handleViewForumResponses(forum._id)}
                                title="Voir les réponses du forum"
                              >
                                <FaEye />
                              </Button>
                              <Button
